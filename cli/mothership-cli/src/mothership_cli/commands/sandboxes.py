...
from mothership_client.models.sandbox import CreateSandboxInput, Sandbox, SearchSandboxesInput
...
class SandboxesStop(BaseModel):
    ...
    sandbox_id: CliPositionalArg[str] = Field(description="Sandbox to stop")

    def cli_cmd(self) -> None:
        client = get_client()
        try:
            client.stop_sandbox(self.sandbox_id)
        except ApiError as e:
            raise SystemExit(str(e)) from e
        if is_json_output():
            print(json.dumps({"sandbox_id": self.sandbox_id, "status": "stopped"}, indent=2))
        else:
            print(f"Stopped sandbox '{self.sandbox_id}'")
